...
const TableFooter = forwardRef<HTMLTableSectionElement, HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <tfoot
      ref={ref}
      className={cn('border-t bg-dark-300 font-medium [&>tr]:last:border-b-0', className)}
      {...props}
    />
  )
)
...
const TableRow = forwardRef<HTMLTableRowElement, HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'border-b border-dark-300 transition-colors hover:bg-dark-300/50 data-[state=selected]:bg-dark-300',
        className
      )}
      {...props}
    />
